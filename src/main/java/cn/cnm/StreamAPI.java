...
public class StreamAPI {
    public static void main(String[] args) {
        ...
        // 可以返回元素对象的某一个属性、将元素的具体某个元素做一系列操作再返回等操作，然后所有返回的结果组合成一个新的流
        System.out.println("集合的所有元素都转为大写：");
        list1.stream().map(String::toUpperCase).forEach(System.out::println);
        // flatMap(Function f)：接收一个函数作为参数，将流中的每个值都换成另一个流， 然后把所有转换后的流连接成一个新的集合流
        /*
         * map类似于集合中的add()方法：不管怎么操作都是将返回的结果组合成一个新的集合流
         * flatMap()类似于addAll()方法：和map()方法同样都是作用于每个元素， 但是生成的结果每一个元素就是一个新的集合流
         *  最终返回由N个集合组成的集合
